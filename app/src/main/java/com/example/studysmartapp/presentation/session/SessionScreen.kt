...
private fun SessionScreen(
    ...
    val currentTimersState by timerService.currentTimerState
            ...
            item {
                ButtonsSection(
                    modifier = Modifier
                        ...
                        .padding(12.dp),
                    startButtonClick = {
                        Log.d("SessionScreen", "startButtonClick running")

                        if (state.subjectId != null && state.relatedToSubject != null) {
                            Log.d("SessionScreen", "startButtonClick: if statement")
                            ServiceHelper.triggerForegroundService(
                                context = context,
                                action = if (currentTimersState == TimerState.STARTED){
                                    Log.d("SessionScreen", "startButtonClick: if statement => action: if")
                                    ACTION_SERVICE_STOP
                                } else {
                                    Log.d("SessionScreen", "startButtonClick: if statement => action: else")
                                    ACTION_SERVICE_START
                                }
                            )
                            timerService.subjectId.value = state.subjectId
                        } else {
                            Log.d("SessionScreen", "startButtonClick: else statement")
                            onEvent(SessionEvent.NotifyToUpdateSubject)
                        }
                    },
                    cancelButtonClick = {
                        Log.d("SessionScreen", "cancelButtonClick running")
                    ...
                    finishButtonClick = {
                        Log.d("SessionScreen", "finishButtonClick running")
...
private fun ButtonsSection(
    ...
    Row(
        ...
        // CANCEL BUTTON     // Doubt at enabled
        Button(
